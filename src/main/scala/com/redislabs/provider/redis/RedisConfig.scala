...
/**
  ...
  * db number, and timeout and ssl mode
  *
...
case class RedisEndpoint(host: String = Protocol.DEFAULT_HOST,
  ...
  /**
    ...
    * spark.redis.db and spark.redis.ssl
    *
  ...
  /**
    ...
    */
  def this(uri: URI) {
    this(uri.getHost, uri.getPort, JedisURIHelper.getPassword(uri), JedisURIHelper.getDBIndex(uri), Protocol.DEFAULT_TIMEOUT, uri.getScheme == RedisSslScheme)
  }
